...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        match_fields={
            "hdr.myTunnel.dst_id": tunnel_id #匹配tunnel_id
        },
        action_name="MyIngress.myTunnel_forward",
        action_params={
            "port": SWITCH_TO_SWITCH_PORT #下发两个交换机连接的接口
        })
    ingress_sw.WriteTableEntry(table_entry)
...
def readTableRules(p4info_helper, sw):
    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            table_name = p4info_helper.get_tables_name(entry.table_id) #获取table_id
            print('%s: ' % table_name, end=' ') #打印table_name
            for k in entry.match: #在每个匹配项对k进行打印操作
                print(p4info_helper.get_match_field_name(table_name, k.field_id), end=' ')
                print('%r' % (p4info_helper.get_match_field_value(k),), end=' ')
            action = entry.action.action
            action_name = p4info_helper.get_actions_name(action.action_id)
            print('->', action_name, end=' ')
            for n in action.params:#在每个动作中的参数对n进行打印操作
                print(p4info_helper.get_action_param_name(action_name, n.param_id), end=' ')
                print('%r' % n.value, end=' ')
            print()
            print(entry)
            print('-----')
